...
describe("lawmind-fleet-actions", () => {
  ...
  it("drops cross-session chat actions when the row has no session binding (avoid approving the wrong ticket)", () => {
    const out = collectFleetActions({
      summary: {
        chatRequiresActions: [
          {
    ...
    const ids = out.map((a) => a.id);
    expect(ids).toEqual(["ap-bound"]);
  });

  it("pickFleetActionForRun binds by actionId first, then approvalId", () => {
    const actions = [
      {
        id: "act-1",
        kind: "tool_approval",
    ...
    expect(pickFleetActionForRun(actions, { actionId: "act-1" })?.id).toBe("act-1");
    expect(pickFleetActionForRun(actions, { approvalId: "ap-2" })?.id).toBe("ap-2");
    // actionId 命中时优先于 approvalId。
    expect(pickFleetActionForRun(actions, { actionId: "act-1", approvalId: "ap-1" })?.id).toBe(
      "act-1",
    );
    // 无绑定返回 null（调用方回退池内首个）。
    expect(pickFleetActionForRun(actions, { actionId: "missing" })).toBeNull();
    expect(pickFleetActionForRun(actions, {})).toBeNull();
    expect(pickFleetActionForRun(actions, null)).toBeNull();
  });
});
