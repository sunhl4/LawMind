...
> **状态**：工程评审（含落地修复）· 2026-07-12 基线；**2026-07-28 晚间独立复评校正**（工程可合并 **≈8.7/10**，非自评 9.5）；现行入口见 [`LAWMIND-FUTURE-ISSUES.md`](../LAWMIND-FUTURE-ISSUES.md) 与 [`LAWMIND-SIMPLE-RELIABLE-PLAN.md`](LAWMIND-SIMPLE-RELIABLE-PLAN.md)  
...
**现行计划入口**：[`LAWMIND-FUTURE-ISSUES.md`](../LAWMIND-FUTURE-ISSUES.md)（长期回看）+ [`LAWMIND-SIMPLE-RELIABLE-PLAN.md`](LAWMIND-SIMPLE-RELIABLE-PLAN.md)（铁律改动）；外接权威语料为 Track B。
...
| R-P0-7 | ~~**评分诚实 / 基线校正**~~ **文档已校正**       | 本文件 §0；未完项见 FUTURE-ISSUES / SIMPLE-RELIABLE-PLAN                                                                      |
...
> **现行入口**：[`LAWMIND-FUTURE-ISSUES.md`](../LAWMIND-FUTURE-ISSUES.md) 与 [`LAWMIND-SIMPLE-RELIABLE-PLAN.md`](LAWMIND-SIMPLE-RELIABLE-PLAN.md)；勿再按下列历史 T0–T4 开新平行线。
...
| UI 半暴露接线（专案组/compact）                       | 2026-07-21 | playbook/cancel/parallel；compose compact；记忆 suggest / finalize / desk-settings / profile-sections / triage rules / quality JSON。Solo「驾驶舱」首页已移除（默认对话）。                                                                                                                       |
...
| R-P0-7 评分基线校正                                   | 2026-07-25 | §0 改为独立复评 ≈7.9 综合 / ≈8.2–8.4 工程可合并                                                                                                                                                                                                                                                   |
...
| WARN | 「工程 9.5」文档超前                 | 本文件旧 §0；已校正；现行入口见 FUTURE-ISSUES / SIMPLE-RELIABLE-PLAN                             |
...
### 附录 · 2026-08-13（独立复评 WARN 落地轮）

> **方法**：2026-08-13 独立全工程评审后按 P1 稳态项落地；不拆神模块、不整理脏树 PR。

| 级   | 项                                  | 处置                                                                      |
| ---- | ----------------------------------- | ------------------------------------------------------------------------- |
| WARN | 邮件短路径默认创建 interval=30min   | ✅ `mailShortPathCreateSchedule` → `once`；跑完 runner 仍 `enabled:false` |
| WARN | `markDelegationTimeout` 无终态守卫  | ✅ 对齐 failed/cancel；completed 后 timeout 不再翻转                      |
| WARN | jobs GET/cancel 不校验 workspaceDir | ✅ 与 SSE 同一 `jobBelongsToWorkspace`；跨工作区 404                      |
| WARN | `createSession` 非原子写            | ✅ `writeJsonAtomic`                                                      |
| WARN | 自动办件 JSON 直写无锁              | ✅ `saveAutomation` / inbox 原子写 + 文件锁                               |
| WARN | 侧栏会话 `button role=listitem`     | ✅ `listbox` + `option` + `aria-selected`                                 |
| WARN | 对话加载无 `aria-busy`              | ✅ 消息区 `aria-busy={loading}`（角色仍 `region`，避免拆 e2e locator）    |
| WARN | 研究快车道 e2e 未进 PR              | ✅ `solo-research-fast-lane.spec.ts` 列入 `e2e:pr`                        |
| INFO | 委派 persist 非原子                 | ✅ `persistRecord` 改 `writeJsonAtomic`                                   |

**仍未修（有意缓做）**：脏树拆 PR（N-A0，组织动作）；`engine-pipeline-tools` 按工具族拆文件（下次改该文件时再拆）；邮件密钥钥匙串（FUTURE）；消息区保持 `region`（不改 tabpanel）。

**2026-08-14 · 会话并行 turn**：`withSessionTurnGate` 按 workspace+session 串行 `runTurn` / `resumeTurn` / `resumePausedTurn`，避免同会话后写覆盖。

### 附录 · 2026-08-14 · DeepSeek 三刀（律师可见过程 / 可停 / 可补材料）

借框架、不抄编码 agent。邮件读取 / 改稿 / Word 交付的 `execute` 未改，只改过程呈现与取消/注入/回放。

| 刀       | 落地                                                                                                                                                                |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 中文卡 | `tool-lawyer-card.ts`；轨迹/live-turn 用「起草文书 / 查看邮件匣 / 按词修订」等，不露 snake_case                                                                     |
| 1 Stop   | `timeoutMiddleware` 合并 turn AbortSignal + 超时；`ctx.abortSignal` 绑本轮 Stop；模型 fetch 原本已取消；`executeToolBatches` 对未启动工具补「已停止」结果、不再开跑 |
| 1 注入   | `POST /api/sessions/:id/inject` → `*.pending-pins.json` 侧车；每轮模型调用前 claim，不重开 turn                                                                     |
| 2 回放   | `sessions/<id>.events.jsonl` 追加结构事件（不含 token delta）；`GET live-turn` 内存空则从日志投影                                                                   |
| 3 段表   | `SYSTEM_PROMPT_SECTION_CATALOG` + Doctor「助手说明装配」；`buildSystemPrompt` 正文不变                                                                              |

**仍未做**：用事件日志替换 `session.json`；Cordis 插件核；delta 落盘。

### 附录 · 2026-08-16 · DeepSeek 第二遍四刀（纠正本轮 / spill / 溢出回收 / 停止正交）

借 lifecycle / compaction / spill，不搬 Goal、Ralph、Cordis。邮件读取 / 改稿 / Word 交付的 `execute` 未改。

| 刀         | 落地                                                                                                                                                                  |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 4 纠正本轮 | `POST /api/sessions/:id/steer` → `*.pending-steer.json`；与 pins 一样在下一轮模型请求前 claim。进行中 Enter /「发送」走 steer；排队整轮在 +「下一轮再发」，不占主按钮 |
| 6 spill    | 截断 research / `list_mail_*` / `read_*` / `analyze_document` 时全文进 `sessions/<id>.spills/<callId>.json`；写稿/红线/导出/发信不 spill                              |
| 5 溢出回收 | 模型上下文溢出时先剪已落盘工具结果（守配对），同一轮重试一次；默认不上 LLM 摘要                                                                                       |
| 7 停止正交 | `timeoutMiddleware` 返回 `{ aborted }` / `{ timedOut }`，不再用抛错混在一起；卡片「已停止 / 已超时」                                                                  |

**仍不做**：Goal 域、events.jsonl 升格权威、token 重放、workflow 脚本。

### 附录 · 2026-08-16 · 律师工作 OS W1（校验 / 缓存边界 / 工具披露 / 本件 overlay）

借 Claude Code / Codex 的循环内核验与前缀缓存，不搬 Goal / Ralph。邮件读取 / 改稿 / Word 交付的 `execute` 未改。

| 切片 | 落地                                                                                                                                |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------- |
| W1-A | `legal-verify-middleware` 插在 argSchema 与 audit 之间：拟发特权/域外硬拦；草稿引用缺失软回写；渲染失败只抄教练                     |
| W1-B | `buildSystemPromptParts` + `---LAWMIND_PROMPT_DYNAMIC_BOUNDARY---`；`applySystemPromptToHistory` 保持已发静态前缀；epoch 写入静态段 |
| W1-C | 默认 verbosity=`compact`；`CORE_MODEL_TOOL_NAMES`（12）+ `list_more_tools`；本会话披露不改静态前缀                                  |
| W1-D | `src/lawmind/work/` overlay；persist 缝 upsert；`POST /api/works` / goal；steer 旁领取目标。无新芯片、无新页                        |
| W1-E | 在办仍四 Tab；队列按 `workId` 一行一事。律师文案不改                                                                                |

### 附录 · 2026-08-16 · 律师工作 OS W2（交付物路由 / 只读对比）

律师只打字或丢文件。规格与对比在引擎，不加页面、不加 chips。

| 切片    | 落地                                                                                                                                          |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| W2 规格 | `lawyer-work-specs.ts` 并入内置 registry：律师函/回函、起诉状/答辩状/代理词、意见书/备忘、时间线/证据目录、会议纪要、NDA                      |
| W2 路由 | `detectDeliverableType` + keyword-route：催款函仍 `letter.demand`；「起草一份律师函」→ `letter.counsel`；「写起诉状」→ `litigation.complaint` |
| W2 对比 | `compare_documents` 只读，复用 `analyze_document` ingest；`list_more_tools` 披露；不进核心 12                                                 |

**仍不做**：真源法宝联调、脏树拆 PR、events.jsonl 升权威（W4）、邮件确认改开 work、对比台/起诉状 chips。

### 附录 · 2026-08-16 · 律师工作 OS W3（诚实 / 棘轮 / 口径钉 / 本件检索）

全部在引擎。不加签批教学条、不加 Playbook 页。

| 切片 | 落地                                                                                         |
| ---- | -------------------------------------------------------------------------------------------- |
| W3-A | `sourceTier`：工作区法条/类案检索恒为 `sample`；无 live 时意见书/函件/诉讼稿软标「未接真源」 |
| W3-B | `rejection-ratchet`：带 label 的驳回写入 work 事件（引用→verify）；裸驳回不写                |
| W3-C | 合同类指令自动钉 `CLAUSE_PLAYBOOK.md`，律师不用找设置                                        |
| W3-D | `search_matter` / `search_workspace` 扫 `lawmind/works`；`read_case_file` 默认 8k 窗口       |

### 附录 · 2026-08-17 · Solo 壳收敛（对话 UI 不动）

对话消息区 / 空态 / 交办条保持原样。其余按商业级 Solo 壳收敛：

| 面   | 落地                                                                                                    |
| ---- | ------------------------------------------------------------------------------------------------------- |
| 设置 | 默认进「模型与连接」；导航分 工作台 / 知识与助手 / 专业控制（Solo 折叠）/ 关于；Doctor 改称「系统健康」 |
| 顶栏 | Solo 会议室降为次级样式；仅一名助手时不显示助手切换                                                     |
| 在办 | Solo 默认只强调「待拍板」；委派/流程收进「更多」（有进行中交办或已进入该区时仍展开）                    |
| 引擎 | `letter.counsel` / 诉讼 / 意见书等规则底稿对齐验收章节；「出具法律意见」走起草                          |

### 附录 · 2026-08-17 · 改稿场景化（文书台入口收敛）

能力保留（`ReviewWorkbench`：改稿 / 批注 / 验收 / 预览 / 导出）；产品面收敛：

| 面              | 落地                                                             |
| --------------- | ---------------------------------------------------------------- |
| Solo 顶栏       | 不展示「文书台」对等 Tab；深链打开 `review` 时显示次级「改稿」   |
| Firm 顶栏       | 仍保留「文书台」peer Tab（e2e mock 为 firm）                     |
| 在办 / 场景 CTA | 「打开文书台」等统一为「改稿」；签批主路径仍在在办               |
| 对话主 UI       | 未改消息区 / 空态 / compose；命令面板 `/review` 文案改为「改稿」 |

### 附录 · 2026-08-17 · 改稿场景化复审 + 指引对齐
...
| --------------- | ------------------------------------------------------------------------------ |
| e2e             | `改稿预览` 断言改为 `改稿`                                                     |
| 模型/Skill 指引 | 签批→「在办」；全文/导出→「改稿」；不再让模型说 Solo 已消失的「文书台」peer 名 |
| 空在办          | Solo 空态增加「改稿」入口                                                      |
| 通知文案        | 与点击打开改稿页对齐                                                           |
| 残留            | Firm 仍用「文书台」Tab 名（有意保留）                                          |

### 附录 · 2026-08-18 · 改稿文案收敛 + Solo e2e

| 项          | 落地                                                                       |
| ----------- | -------------------------------------------------------------------------- |
| CTA 文案    | 案件概览 / 任务板 / 认知 / 对话状态条等用户可见「改稿预览」→「改稿」       |
| 审计 detail | 写入「打开改稿」；解析兼容旧「打开改稿预览」与「进入审核台/文书台」        |
| Solo e2e    | `review-entry`：route Solo edition → 无 peer Tab；从在办进入后次级「改稿」 |

### 附录 · 2026-08-17 · 设置简化 P0–P3

| 项              | 落地                                                                                            |
| --------------- | ----------------------------------------------------------------------------------------------- |
| P0 系统健康     | 默认：连接 / 工作区真相 / 轻量信任 + 去在办；运维硬控进 `lm-doctor-admin`                       |
| P1 签批叶       | 「签批与导出」并入外观；nav 去叶；`review-prefs` 深链仍渲染外观                                 |
| P1/P2 Solo 导航 | `settingsNavGroupsForEdition`：Solo 隐藏角色说明、团队工作流                                    |
| P2 版本         | Solo：徽章 + 一句 + 单一 `details[data-testid=lm-edition-admin]`；Firm：同 testid 的开放 body   |
| P3              | 模型首屏收起草/Worker；MCP 改文档说明；工作区批次目录 details；自动办件加 lead                  |
| 文档            | `docs/` 与 `apps/lawmind-docs/docs/` 的 USER-MANUAL §2.3–4 已对齐（改稿 / Solo 顶栏 / 设置 IA） |

复审补丁：系统健康「快捷入口」改为「在办」（不再链 Solo 已藏的团队工作流）；设置搜索 Enter 走 `settingsNavGroupsForEdition`。

### 附录 · 2026-08-18 · 铁律闭环

对照当日三通终评（上手 6.5 · 质量 6.3 · 稳定 6.9，综合 6.5），落地 P0–P2 而不改历史自评分数：

| 波次 | 落地                                                                                                                                                                                                                                  |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0   | 催告/催款生成器与路由对齐；`spec.not_found` 不得 ready；合同/函/意见书统一拦【】占位；导出须有 `outputPath`；签批写戳失败禁止自动导出并回 `matterWriteFailed`                                                                         |
| P1   | 空对话只展开合同卡；在办默认展开非空组；律师词（系统健康 / 一键审邮件合同 / 配置模型）；INSTALL/手册/Electron 菜单；Solo 不渲染 Firm 驾驶舱空提示；验收未过禁用一键勾选必核；自动办件失败进 inbox；案件审批可办；9 档 prompt 结构快照 |
| P2   | `contract.review` 风险须条款锚点；就绪条纳入推理门；签批 `expectedReviewStatus` CAS 409；写戳失败条「打开系统健康」；交办可回灌【已采纳偏好】                                                                                         |

`docs/LAWMIND-SIMPLE-RELIABLE-PLAN.md` A1–A4 / B1–B3 / C2–C5 已按本轮实际落地勾选。不加人、不加入口、不把 Insights 做进 Solo 默认。

### 附录 · 2026-08-19 · 全功能树上收口（不回退 Desk 叉）

在 `backup/pre-split-na0` + 铁律工作区上简化默认面，**保留会议室与自动办件**：

| 面     | 落地                                                                     |
| ------ | ------------------------------------------------------------------------ |
| 顶栏   | 「会议室」恢复一级 Tab（不再次级弱化）；改稿仍为场景页                   |
| 空对话 | 合同卡 + 写这封 / 查这个问题；写材料 / 按流程办 / 自动办件进「更多办件」 |
| 在办   | 条上「自动办件」打开设置分区；待拍板 / 交办 / 流程仍在                   |

### 附录 · 2026-08-19 · 三条铁律对照瘦身

对照上手简单 / 交付质量 / 交付稳定，只收默认面、不拆门禁：

| 铁律 | 位置                | 落地                                                             |
| ---- | ------------------- | ---------------------------------------------------------------- |
| 上手 | 对话输入条          | 权限四档退出默认条，放进「+」；先计划仍露「开始执行」            |
| 上手 | 对话草稿条 / 交办卡 | 未签批只留「去签批」；看意见不再并列抢主路径                     |
| 上手 | 在办办理区          | 签批主行：通过 / 需修改 / 改稿；驳回与丢弃进「更多」             |
| 质量 | 改稿侧栏            | 助理自检摘要提到主审阅区；验收/必核/引用仍可见                   |
| 稳定 | 改稿底栏            | 签批后导出与审阅稿仍外露；材料包/文件夹/恢复待审核进「更多交付」 |

### 附录 · 2026-08-19 · 空对话让模型主写

空对话不再展开合同卡 / 动词 / 更多办件。默认只提示直接说话。审这份、写这封、查这个问题、写材料、邮件、自动办件收入输入栏 **办件**；快车道仍叠在输入框上方，门禁与签批不变。

### 附录 · 2026-08-19 · 律师版 Cursor（高级手段，不堆硬控）

对齐 Cursor / Claude Code / Codex 的能力循环，而不是再加芯片或房间：

| 层       | 落地                                                                                              |
| -------- | ------------------------------------------------------------------------------------------------- |
| 路由     | 有 LLM 凭据时 `routeAsync` 默认模型分类；`LAWMIND_ROUTER_MODE=keyword` 可强制回退；失败仍走关键词 |
| 系统提示 | 明确「不要等律师先选通道」；对标编码智能体的工具循环，签批仍在「在办」                            |
| 默认面   | 「办件」文案改为可选快捷；空对话继续直接说话                                                      |
| 未改     | 会议室、自动办件、验收/空交付/危险工具硬门禁                                                      |

### 附录 · 2026-08-19 · 产品化能力（Skill 写成 LawMind 功能）

完全交给模型会飘。高频律师活收成能力目录，本轮自动绑定并注入对应 Skill + 流水线：

| 能力                            | 流水线                     |
| ------------------------------- | -------------------------- |
| 合同审查 / 函件 / 诉讼 / 写材料 | `execute_workflow` + 验收  |
| 检索研究                        | `research_task` → 引用锚定 |
| 邮件合同                        | 短路径 tracked redline     |

落点：`src/lawmind/skills/lawyer-capabilities.ts`，注入 `turn-orchestrator-prompt.ts`。口头答疑不绑定。末端硬门禁不变。

### 附录 · 2026-08-19 · 办件选流程（不靠激活词）

律师不靠记住「审这份 / 查一下」。主路径：附材料 →「办件」列表选流程 → 指令写 `【办件】能力：…` 锁。无材料时合同审查 / 检索仍打开选文件或主题卡片（e2e 黄金路径保留）。

### 附录 · 2026-08-20 · 全工程 vs Desk #35

对照 `backup/pre-split-na0` + 脏树 与 draft PR [#35](https://github.com/sunhl4/LawMind/pull/35)（`cursor/desk-first-cut-5f4f` @ `83f00eb`）。可视化：[lawmind-vs-desk-review-2026-08-20](/Users/shl/.cursor/projects/Users-shl-nvidia-LawMind-1/canvases/lawmind-vs-desk-review-2026-08-20.canvas.tsx)。

| 轴       | 当前树                            | Desk                                   | 裁决            |
| -------- | --------------------------------- | -------------------------------------- | --------------- |
| 产品主线 | 对话 / 在办 / 会议室 + 办件选流程 | Solo 单面 + 三激活词                   | **留当前**      |
| 质量工艺 | 验收门禁在；骨架稿诚实弱          | 密骨架不得 ready；复核不改正文；条款图 | **摘 Desk**     |
| 可合并   | ~591 脏文件                       | 100 文件 draft，与当前未合并           | **先拆当前 PR** |

**不要**整支合 Desk（会丢会议室 / 自动办件，并恢复激活词）。P0 摘：`scaffold-status`、复核笔记纪律。P1：条款图红条、成稿失败文案。P2：Word 侧车 / `lawmindd`。B4 仍开。本轮未复跑全量测试。

### 附录 · 2026-08-20 · 摘 Desk 质量工艺（当前树）

在当前产品线落地，不回激活词、不删会议室 / 自动办件：

| 铁律 | 手段                                      | 落点                                                     |
| ---- | ----------------------------------------- | -------------------------------------------------------- |
| 质量 | 密骨架不得 `ready`；法释括号不误拦        | `scaffold-status` + `validator` `draft.scaffold_density` |
| 质量 | 第二审只加 `复核：` 意见，不改 `sections` | `draft-critic` + 条款图 map-reduce                       |
| 上手 | 骨架稿在对话条 / 改稿页诚实标明           | `lm-draft-status-scaffold` / `lm-review-scaffold-banner` |
| 稳定 | 深度升专案组带走立场/重点                 | `reviewBrief` + 【审查口径】注入 `sourceText`（B4）      |
| 质量 | 模型未成稿标注回退，不假装已成稿          | `MODEL_DRAFT_FALLBACK_NOTE`                              |

未摘：Word 侧车 / `lawmindd`（P2）。末端硬门禁仍只留给空交付、未签批、危险工具。

### 附录 · 2026-08-20 · 律师 IA Review 1–3（只暴露高频）

对照 Cursor / Claude / Codex / Harvey / Spellbook / CoCounsel，收默认面、不删能力、不加判断类硬控：

| 面     | 落地                                                                                         |
| ------ | -------------------------------------------------------------------------------------------- |
| 顶栏   | 常驻对话 / 在办；改稿、文书台、会议室均为场景页（打开后次级定位）                            |
| 办件   | 第一屏：合同审查 / 函件起草 / 检索研究 / 写材料；诉讼、邮件、调度进「更多」                  |
| 输入条 | 常驻 + / 办件 / 发送；模型可留；压缩仅 warn/compact；工具轨迹在设置外观                      |
| 改稿   | 待签批主按钮只留「回到在办签批」；验收/必核/自检一行点开；批注默认收起；条款图无旗标整图折叠 |
| 设置   | 编制 / 记忆 / 技能进专业控制；Solo 侧栏隐藏编制；版本并进关于                                |

会议室与自动办件仍在产品内。2026-08-20 复跑 `pnpm lawmind:desktop:e2e:pr`：53 passed / 2 skipped。

续：Review 1–3 原文剩余项已补：工具轨迹迁设置；改稿头栏不再重复「回到在办」；自检/验收合成核对明细一行；批注默认收起；Solo 侧栏隐藏编制（深链保留）；版本与授权并进关于。未做：Word 侧车 / `lawmindd`（不在这三轮 IA）。

### 附录 · 2026-08-20 · Grok Bot 运行时技巧（学进程，不学云电脑）

对照 Grok Bot 公开实现（MindStudio / Unite.AI / Helio，2026-08）与本树 harness。可视化：[lawmind-grok-bot-runtime-techniques](/Users/shl/.cursor/projects/Users-shl-nvidia-LawMind-1/canvases/lawmind-grok-bot-runtime-techniques.canvas.tsx)。

| 学                                             | 不学                                   |
| ---------------------------------------------- | -------------------------------------- |
| 作业是进程（关窗后续跑）→ 本机 `lawmindd`      | 云 VM 托管客户卷宗                     |
| 只在不可逆处暂停（已有 RequiresAction / 签批） | 共享浏览器会话 / 代登业务站            |
| 办件完成 → 存成自动办件（不录屏）              | 对话里收授权、Computer-use 点法宝/法院 |
| 软 tool 预算 + 检查点续跑                      | 把聊天窗口当状态单位                   |

本树已更好：案件隔离、工具中间件、session turn gate、原子写、本件目标 sidecar、协作产物落盘。三刀已落地（见下一节）。

### 附录 · 2026-08-20 · 落地 Grok 三刀（进程，不是云电脑）

| 刀          | 律师看见                                                   | 落点                                                          |
| ----------- | ---------------------------------------------------------- | ------------------------------------------------------------- |
| 本机守护    | 设置 → 工作区「关桌面后继续办件」；退出桌面拉起 `lawmindd` | `lawmind-daemon.ts`、`LAWMIND_DAEMON=1`、`claimDueAutomation` |
| 办件 → 例行 | 在办签批通过后「存成自动办件」                             | `POST /api/works/automation`、`automation-from-work.ts`       |
| 软预算续跑  | 约 40 步询问「继续 / 先停在这里」；硬顶约 80               | `continue_tools` + `skipToolBudgetCheckpoint`                 |

不上云、不共享浏览器会话、不录屏。`pnpm lawmind:daemon -- status\|start\|stop`。

### 附录 · 2026-08-20 · 独立全工程 + UI 复评（不改历史分数）

对照脏树 + 代码抽检，**不是**门禁复验。可视化：[lawmind-independent-review-2026-08-20](/Users/shl/.cursor/projects/Users-shl-nvidia-LawMind-1/canvases/lawmind-independent-review-2026-08-20.canvas.tsx)。

| 轴           | 今日        | 备注                                                 |
| ------------ | ----------- | ---------------------------------------------------- |
| 盘上工程水位 | **≈8.2/10** | 信任闸 / 办件锁仍在；Grok 三刀加能力也加缝           |
| 整树可合并   | **≈6.4/10** | `git status` **637** 路径（448 M / 168 ?? / 21 D）   |
| 产品 IA      | **≈8.0/10** | 顶栏对话/在办对；空态「+」与在办「批准」错位         |
| 信任闸门     | **≈8.6/10** | 澄清挡写、危险批准、空交付、打包忽略 `SKIP_API_AUTH` |

**不要**把 §0 的工程 ≈8.9 / 历史 9.5 当成今天的树。本附录不改写上文分数。

上文同日附录 683 / 699 仍写「未做 Word 侧车 / `lawmindd`」——那是 **IA / Desk 摘取当时的残项**。同日后文 712–722 已写三刀落地。以落地附录为准；`lawmindd` 已在树里，Word 侧车仍是 P2。

**已核实 WARN（置信 ≥75）**

1. `processDueScheduledJobs`（`lawmind-server-jobs.ts`）无认领锁；`scheduled` 经 `loadJobsFromDiskOnStartup` 重启仍在。设置可在桌面开着时「立即启动」守护，两进程各自 tick。`claimDueAutomation` 只管自动办件。
2. `continue_tools` 批准走新 `runTurn`（`runtime-resume.ts`）：`toolCallsExecuted` 从 0 起且 `skipToolBudgetCheckpoint: true`，硬顶约再 80 步。
3. 空对话「拖入或 +」（`lawmind-chat-messages-column.tsx`）；`+` 的 `aria-label` 是「输入选项」，附材料入口是「办件」/拖入。
4. 在办 `continue_tools` 卡片 `hideActions`，dock 主按钮仍是「批准」（`LawmindAgentFleetPanel.tsx`）。
5. 设置/成功句使用「本机守护」；项目文件夹「关闭」= 清除文件夹。
6. 守护 `spawn` 复制全量 `process.env`（含 `LAWMIND_LOCAL_API_TOKEN`），未走 `collectSecretsForServerEnv`。

**验证诚实**：IA 当日 `e2e:pr` 53/2 是三刀之前。三刀后未复跑全量 `pnpm test` / `e2e:pr`。`GOALS.md` 仍写 38/2/0。不得宣称门禁全绿。

本轮**只评审、不改代码**。下一轮 P0：空态/在办文案、预约 jobs 认领或互斥 tick、continue 累计工具次数。

### 附录 · 2026-08-20 · 独立复评 P0/P1 已落地

对照上一节 WARN，本轮已改（不拆 N-A0、不复跑全量门禁）：

| 项                            | 落点                                                      |
| ----------------------------- | --------------------------------------------------------- |
| 空态改「拖入或点办件」        | `lawmind-chat-messages-column.tsx`                        |
| 在办 continue 继续/先停       | `fleetApprovalDockLabels` + dock `lm-continue-tools-*`    |
| 去「本机守护」/文件夹「清除」 | `LawmindSettingsWorkspace.tsx`；桌面开着不再「立即启动」  |
| 预约 jobs 认领锁              | `claimDueScheduledJob`；tick 会从磁盘补 scheduled         |
| 桌面↔lawmindd 互斥            | 桌面启动 `stopDaemonProcess`；退出先停 HTTP 再拉守护      |
| continue 累计工具次数         | `initialToolCallsExecuted`；软检查点仍 skip，硬顶按累计   |
| 守护 env 白名单               | `buildDaemonProcessEnv`；`SECURITY.md` 增补 lawmindd 一节 |

残留：Word 侧车 P2；脏树 N-A0；全量 `pnpm test` / `e2e:pr` 未在本轮复跑。
