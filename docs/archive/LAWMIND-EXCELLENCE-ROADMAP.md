...
- 合成回归 fixtures（结构/lint 回归层）：`src/lawmind/evaluation/replay-fixtures.ts` 内置 12 个合成法律任务样本，覆盖合同审查、律师函、客户更新、证据目录、案件时间线和法律 memo（口径：非真实任务回放，真实已结案回放见 GOALS.md 14.11 人/数据债）。
- 发布报告：`src/lawmind/evaluation/release-report.ts` 汇总黄金旅程、合成回归 fixture、benchmark、quality dashboard 和验证命令。
