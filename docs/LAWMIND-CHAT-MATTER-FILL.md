...
0. **写了就要看见（新增，2026-09-19）。** 抽取到的事实必须落到**界面确实读取的那份存储**。只打 `confirmedAt`、把当事人/案由留在 `intake-brief.json` 里等于没写：工作台卷宗读的是 `matter.json`。对标的额、保全这类原先无字段项，按「时间敏感进提醒系统、数值字段照原文存」落地——保全 → `preservation` 期限（有提醒），标的金额 → `docket.claimAmount`（自由文本，不解析）。参考 Codex/Claude Code 的教训：静默丢弃「承重字段」会让模型在可见前缀上编造。
