...
### 1.1) 产品信息架构（2026-07-13，Wave A/B 2026-07-19 修订；2026-07-21 去掉 Solo 驾驶舱首页）

- **默认入口**：打开即 `workspace` 对话。待拍板 / 专案组等从侧栏「待我拍板」与「在办」进入，不再单独占一级「驾驶舱」页。
- **一级顶栏**：「对话」「在办」「会议室」（条件出现：案件 chip / 文书台场景标签）。
- **状态层 / 在办**：`agents` 对用户显示为「在办」= **领导视图 + 拍板台**（不是又一个案件浏览器）。
  - **顶部分区（常驻）**：**待拍板** · **交出去的活** · **按流程办**（同一页内切换，不再靠设置「协作」当日常入口）。
  - **待拍板布局**：隐藏全局案件侧栏（与文书台同理）；页内两栏 = **左：团队 / 队列**（默认「团队」按助手聚合：忙闲、近 30 日一次过、待教；「队列」为待签批 / 待补充 / 待批准下钻，可按案件筛选）· **右：办理区**。顶栏「待教团队」可进设置→记忆；「协作完成」为近 48h 互审/委派信息角标（不计入待拍板）。
...
- **交付层**：`review` 对用户显示为「文书台」，不再是顶栏对等 Tab；由「进入文书台」场景按钮打开。**文书台**负责改稿、批注与交付预览；**正式通过 / 驳回 / 需修改**在「在办」完成（无需打开全文预览）。文书台底部「回到在办签批」；必核清单勾完后也可在文书台高级区兜底签批。
