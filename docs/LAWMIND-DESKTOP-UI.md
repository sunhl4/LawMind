...
相关文档：[使用手册（完整版）](/LAWMIND-USER-MANUAL)（桌面优先）、[LawMind 架构](/LAWMIND-ARCHITECTURE)（§十一 b 桌面应用）、[LawMind 2.0 strategy](/LAWMIND-2.0-STRATEGY)（产品级智能化与记忆图）。
...
### 1.1) 产品信息架构（2026-07-13，Wave A/B 2026-07-19 修订；2026-07-21 去掉 Solo 驾驶舱首页；2026-07-24 文书台曾恢复为顶栏对等 Tab；2026-08-17 Solo 改稿场景化）

- **默认入口**：打开即 `workspace` 对话。空对话先附材料，再点「办件」选流程（不必记激活词）。待拍板 / 专案组等从侧栏「待我拍板」与「在办」进入，不再单独占一级「驾驶舱」页。
- **一级顶栏（Solo）**：「对话」「在办」「会议室」。**改稿**（原「文书台」）不占一级对等 Tab，从在办「改稿」/ 对话深链进入；已打开时顶栏显示次级「改稿」便于定位。自动办件从「在办」条进入设置页，不占顶栏对等 Tab。
- **一级顶栏（Firm）**：「对话」「在办」「文书台」「会议室」（会议室可按场景条件出现）；Firm 仍保留「文书台」对等 Tab 便于团队入口。
- **状态层 / 在办**：`agents` 对用户显示为「在办」= **领导视图 + 拍板台**（不是又一个案件浏览器）。
  - **顶部分区（常驻）**：**待拍板** · **交出去的活** · **按流程办**（同一页内切换，不再靠设置「协作」当日常入口）。
  - **待拍板布局**：隐藏全局案件侧栏（与改稿页同理）；页内两栏 = **左：团队 / 队列**（默认「团队」按助手聚合：忙闲、近 30 日一次过、待教；「队列」为待签批 / 待补充 / 待批准下钻，可按案件筛选）· **右：办理区**。顶栏「待教团队」可进设置→记忆；「协作完成」为近 48h 互审/委派信息角标（不计入待拍板）。
  - **律师在此做什么**：签批文书、补信息让任务继续、批准/驳回工具写入；看交办进度；按模板跑多步流程。案件材料与对话仍回「对话 / 案件工作台」。Firm 设置→「团队工作流」仅为状态摘要；Solo 从系统健康「打开在办」进入。
...
- **调度层**：自动办件（定时/邮件触发）从 **在办 → 自动办件** 进入设置页配置；与对话内「下达」区分；结果进「待我拍板」。
- **案件层**：案件是材料、任务、期限、交付物和保密等级的长期真相源；在办顶栏可用「全部案件 / 本案」筛选队列，不把案件目录再占一列。
- **交付层**：`review` 场景页负责全文改稿、批注、验收检查与交付预览（能力保留，入口收进在办/深链）。**正式通过 / 驳回 / 需修改**主路径在「在办」完成（无需打开全文预览）。改稿页底部「回到在办签批」；必核清单勾完后也可在改稿页高级区兜底签批。
- **工具批准「改拟稿」**：仅改短字段；拟写入整篇 `content` 时在办只阅读 + 批准/驳回（或「改参数」改路径），全文改稿去改稿页，避免与签批流重合。
- **决策入口**：对话页侧栏底部「待我拍板」为常驻入口（顶栏不再重复角标）。进入「在办」后目录即拍板队列；**无独立 Action Hub 模态**；不再在对话输入区挂「待你处理」条。
- **禁止模式开关**：不增加 IDE/Agent 或默认首页设置；Solo/Firm 只渐进改变信息密度（Solo 设置「专业控制」默认折叠）。
- **设置 IA（2026-08-17）**：默认进「模型与连接」。工作台 = 模型 / 工作区 / 外观（含签批后自动导出）；知识 = 记忆 / 技能 / 助手 / 模板；专业控制（Solo 折叠）= 系统健康 / 自动办件 / 安全与工具 / 版本与授权（Solo 侧栏不含「角色说明」「团队工作流」，深链仍可用）；关于 = 更新 / 免责。系统健康默认瘦身，运维硬控在「运维与高级体检」。
...
- **2026-08-17**：Solo 顶栏去掉「文书台」对等 Tab；改稿为场景页（在办「改稿」/深链进入）；Firm 仍保留「文书台」Tab。能力（改稿/预览/验收/导出）保留。
...
`LawmindMainView`：`workspace`（默认对话）| `agents`（在办）| `meeting`（会议室）| `review`（改稿场景页）— 见 `apps/lawmind-desktop/src/renderer/lawmind-main-view.ts`。自动办件为设置分区（`settings` → `automations`）。协作能力已并入「在办」子区，不再是顶栏对等视图。
