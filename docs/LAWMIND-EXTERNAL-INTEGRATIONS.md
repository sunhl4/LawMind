...
> **关联**：[LAWMIND-OPTIMIZATION-BACKLOG.md](./LAWMIND-OPTIMIZATION-BACKLOG.md)（P0-1 权威）、[LAWMIND-FUTURE-ISSUES.md](./LAWMIND-FUTURE-ISSUES.md)（Track B）、[LAWMIND-ENGINEERING-REVIEW.md](./LAWMIND-ENGINEERING-REVIEW.md)、[LAWMIND-EDITION-FEATURE-MATRIX.md](./LAWMIND-EDITION-FEATURE-MATRIX.md)
...
1. **权威（默认）**：`LAWMIND_AUTHORITY_PROVIDER=open` → `src/lawmind/retrieval/providers/open-law/`（本地 JSONL sample + 可选外部语料；可选 NPC FLK / 自建 caseopen / CourtListener / EUR-Lex / e-Gov 直播）。无命中仍拒答。
...
LAWMIND_OPEN_LAW_MODE=local|npc_flk|caseopen|courtlistener|eurlex|egov_jp|hybrid
...
LAWMIND_OPEN_LAW_COURTLISTENER=0|1            # 1 美国判例（含 Harvard CAP 历史语料）
LAWMIND_OPEN_LAW_COURTLISTENER_TOKEN=         # 可选；提高 CourtListener 限额
LAWMIND_OPEN_LAW_EURLEX=0|1                   # 1 欧盟 CELLAR SPARQL
LAWMIND_OPEN_LAW_EGOV_JP=0|1                  # 1 日本 e-Gov 法令 API
```
...
- Doctor / 设置页探测：对 `open` 做本地语料就绪检查（非厂商付费核验）；`sample-ready` = 演示 sample，`configured` = 含外部 CORPUS；摘要含 `openSources[]`（sample / CORPUS / NPC / caseopen / CourtListener / CAP-via-CL / EUR-Lex / e-Gov）；Lexis 占位为 `unimplemented`（勿与端点 `invalid` 混淆）。
...
| U0d | （可选）启用涉外开源直播                                               | `COURTLISTENER=1`（建议加 Token）/ `EURLEX=1` / `EGOV_JP=1`；CAP 勿再调 `api.case.law` |
