...
export function createPinnedAuthorityFetch(opts?: {
  ...
  return async (input: FetchInput, init?: FetchInit): Promise<Response> => {
    ...
    try {
      ...
      throw new Error(`权威端点 DNS 解析失败（fail-closed）：${detail}`);
    }
    if (!addrs.length) {
    ...
    return new Promise<Response>((resolve, reject) => {
      const req = isHttps
        ? https.request(url, { method, headers: headers as unknown as Record<string, string>, agent }, onResponse)
        : http.request(url, { method, headers: headers as unknown as Record<string, string>, agent }, onResponse);
      ...
      function onResponse(res: http.IncomingMessage): void {
        ...
        res.on("end", () => {
          ...
          for (const [k, v] of Object.entries(res.headers)) {
            if (Array.isArray(v)) {
              for (const item of v) {
                if (item != null) respHeaders.append(k, item);
              }
            } else if (v != null) {
              respHeaders.set(k, v);
            }
