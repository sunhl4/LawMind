...
export const LAWYER_WORK_SPECS: readonly DeliverableSpec[] = Object.freeze([
  ...
  },
  {
    ...
    requiredSections: [
      ...
      { headingKeywords: ["证据", "证明"], purpose: "证据对照", severity: "blocker" },
  ...
  },
  {
    type: "memo.research",
    displayName: "检索研究备忘",
    description: "检索备忘：命题、现行法条、正反类案、结论与缺口。",
    defaultTemplateId: "word/legal-memo-default",
    defaultOutput: "docx",
    defaultRiskLevel: "medium",
    requiredSections: [
      { headingKeywords: ["事项", "问题", "争点"], purpose: "事项", severity: "blocker" },
      { headingKeywords: ["命题", "查询"], purpose: "命题矩阵", severity: "blocker" },
      { headingKeywords: ["法条", "现行"], purpose: "现行法条", severity: "blocker" },
      { headingKeywords: ["正向", "支持"], purpose: "正向类案", severity: "blocker" },
      { headingKeywords: ["反向", "排除", "对方"], purpose: "反向类案", severity: "blocker" },
      { headingKeywords: ["结论", "意见"], purpose: "结论", severity: "blocker" },
    ],
    acceptanceCriteria: ["须含命题、现行法条、正反类案与结论；无命中也保留栏目并标待核实。"],
    placeholderRule: { pattern: PLACEHOLDER_PATTERN, mustResolveBeforeRender: false },
    defaultClarificationQuestions: [],
  },
  {
    type: "labor.calc",
    displayName: "劳动补偿计算",
    description: "经济补偿、加班、双倍工资：公式与数字来自 calculate 工具。",
    defaultTemplateId: "word/legal-memo-default",
    defaultOutput: "docx",
    defaultRiskLevel: "medium",
    requiredSections: [
      { headingKeywords: ["定性", "解除", "工时"], purpose: "定性", severity: "blocker" },
      { headingKeywords: ["计算", "公式"], purpose: "计算", severity: "blocker" },
      { headingKeywords: ["结论", "金额"], purpose: "结论", severity: "blocker" },
    ],
    acceptanceCriteria: ["金额必须带来源公式；缺流水标缺口，不得口算假数。"],
    placeholderRule: { pattern: PLACEHOLDER_PATTERN, mustResolveBeforeRender: false },
    defaultClarificationQuestions: [],
  },
  {
    type: "period.calc",
    displayName: "程序期限计算",
    description: "上诉、答辩、仲裁、执行期间届满日。",
    defaultTemplateId: "word/legal-memo-default",
    defaultOutput: "docx",
    defaultRiskLevel: "medium",
    requiredSections: [
      { headingKeywords: ["起算", "送达"], purpose: "起算", severity: "blocker" },
      { headingKeywords: ["届满", "期限"], purpose: "届满日", severity: "blocker" },
    ],
    acceptanceCriteria: ["届满日必须有公式；中断顺延标缺口。"],
    placeholderRule: { pattern: PLACEHOLDER_PATTERN, mustResolveBeforeRender: false },
    defaultClarificationQuestions: [],
  },
]);
